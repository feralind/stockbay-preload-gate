...
function Fetch-YahooCandles($sym, $resolution, $count) {
  ...
  try {
    ...
    for ($i = 0; $i -lt $ts.Count; $i++) {
      ...
      $close = $q.close[$i]
      $volume = $q.volume[$i]
      # Prefer adjclose on long history so splits don't zig-zag MAX/1Y
...
# Port already in use? Prefer a fresh process so launch never sticks to a stale serve.
$portBusy = $false
try {
...
if ($portBusy) {
  ...
  if ($okServer -and -not $ForceRestart) {
  ...
  Write-Info "Restarting StockWay server on port $Port..." "Yellow"
